...
StructuredBuffer<int> EmitterIdBuffer <string uiname="Emitter ID Buffer";>;
StructuredBuffer<float> LifetimeBuffer <string uiname="Lifetime Buffer";>;
...
void CS_Emit(csin input)
{
	...
	if ( position.z > 0 && !(texUvColor.x <= 0 || texUvColor.x > 1 || texUvColor.y <= 0 || texUvColor.y > 1)){
		...
		p.lifespan = LifetimeBuffer[0];
		
		//EmitterIdBuffer.GetDimensions(size,stride);
		p.emitterid = EmitterIdBuffer[0];
