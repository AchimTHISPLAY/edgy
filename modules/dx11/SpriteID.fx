...
StructuredBuffer<float> ScaleBuffer <string uiname="Scale Buffer";>;
...
VSOut VS(VSIn In)
{
	...
	uint i = 0;
	i = ParticleBuffer[particleIndex].emitterid;
	Out.emitterIndex = i;
	Out.PosW = mul(p, tW);
	...
	switch( i ) {
		...
		case 2 : size = ScaleBuffer[i]; break;
		case 3 : size = ScaleBuffer[i]; break;
...
float4 PS(VSOut In): SV_Target
{
	...
	switch( i ) {
		...
		case 2 : col = ColorBuffer[i]; break;
		case 3 : col = ColorBuffer[i]; break;
		//case 4 : col = ColorBuffer[i]; break;
		case 4 : break;
	...
	*/
	...
	//coll *= texture2d.Sample( sL, float3(In.TexCd.xy, i), 0);
